test_that("runOwiSliceQuery builds the default OWI command", {
  ...
  expect_equal(cmd$args[1:4], c("query", "slice", "-R", "all:latest/collectionName=legal"))
...
test_that("runOwiSliceQuery parameterizes OWI query slice options", {
  ...
  expect_equal(cmd$args[1:4], c("query", "slice", "-R", "all/collectionName=other"))
  expect_equal(cmd$args[match("--where", cmd$args) + 1], "valid=true")
  expect_equal(cmd$args[match("--select", cmd$args) + 1], "id,\nurl")
  expect_equal(cmd$args[match("--collection", cmd$args) + 1], "other_collection")
...
test_that("runOwiSliceQuery quotes shell-sensitive system arguments", {
  ...
  expect_true(any(grepl("ows_index=true OR ows_index IS NULL", output$result, fixed = TRUE)))
})
...
test_that("runOwiSliceQuery returns the latest parquet glob for the collection", {
  ...
  unrelated_newer_dataset <- file.path(local_base_path, collection, "unrelated-newer-dataset")
...
test_that("runOwiSliceQuery validates inputs", {
  ...
  expect_error(runOwiSliceQuery(url_suffix = "", dry_run = TRUE), "`url_suffix`")
  expect_error(runOwiSliceQuery(select = character(), dry_run = TRUE), "`select`")
...
test_that("searchOwi searches main_content by default", {
  ...
  arrow::write_parquet(
    data.frame(
      id = c("1", "2", "3"),
      url = c("https://a.at/impressum", "https://b.at/privacy", "https://c.at/contact"),
