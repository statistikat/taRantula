...
Data is managed through a hybrid architecture: metadata is stored directly in **DuckDB**, while raw HTML content (`src`) is archived in highly compressed **Parquet** files.

The `$results()` method abstracts this complexity by automatically joining metadata with the corresponding Parquet archives. For large datasets, you can toggle the `with_src` parameter to choose between high-performance metadata analysis and full-content extraction.
...
# Access all discovered links from the link graph
links_dt <- s$links()
